...
namespace TribleZ
{
	...
	std::pair<float, float> Editor_Camera::PanSpeed() const		//平移速度
	{
		...
		float x = std::min(m_ViewportWidth / 1000.0f, 2.4f);	//最大为2.4		标准库std里面的max和min真的很奇怪
		//float xFactor = 0.0366 * (x * x) - 0.1778f * x + 0.3021f;
		float xFactor = 0.0966 * (x * x) - 0.1778f * x + 0.3021f;

		float y = std::min(m_ViewportHeight / 1000.0f, 2.4f);
		//float yFactor = 0.0366 * (y * y) - 0.1778f * y + 0.3021f;
		float yFactor = 0.0966 * (y * y) - 0.1778f * y + 0.3021f;
